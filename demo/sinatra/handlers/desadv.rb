...
class OICEdiDesadvHandler
  ...
  def create_desadv_message(so, test=false)
    ...
    # BGM
    @log.info( @log_pfx + 'desadv reference:' + 'DES' + so.id.to_s )
    bgm = msg.new_segment('BGM')
    bgm.d1004 = 'DES' + so.id.to_s
    bgm.cC002.d1001 = 351
    msg.add(bgm)
    # DTM
    # Planned Delivery Time
    @log.info( @log_pfx + 'planned delivery time:' + (so.date_delivery ? so.date_delivery.strftime("%Y%m%d%H%M") : "") )
    dtm = msg.new_segment('DTM')
    dtm.cC507.d2005 = 171
    dtm.cC507.d2380 = so.date_delivery ? so.date_delivery.strftime("%Y%m%d%H%M") : ""
    dtm.cC507.d2379 = 203
    msg.add(dtm)
    # Msg time
    @log.info( @log_pfx + 'message time:' + Time.now.strftime("%Y%m%d%H%M") )
    dtm = msg.new_segment('DTM')
    dtm.cC507.d2005 = 137
    ...
    nad.d3035 = "BY"
    nad.cC082.d3039 = so.partner_invoice_id.ean13 || "GTIN_UNKNOWN"
    msg.add(nad)
    # Supplier
    nad = msg.new_segment('NAD')
    nad.d3035 = "SU"
    ...
    nad = msg.new_segment('NAD')
    nad.d3035 = "DP"
    nad.cC082.d3039 = so.partner_shipping_id.ean13 || "GTIN_UNKNOWN"
    msg.add(nad)
    # CPS
  ...
  def process_business_object( object_id, filename, log, test=false )
    ...
    unless so
      @log.error( @log_pfx + 'No sales order found for id: ' + object_id)
      @log.info( @log_pfx + 'DESADV skipped' )
      return false      
    end
    create_desadv_message(so, test)
    @ic.validate
